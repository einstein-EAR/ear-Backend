...
const formatPaperSubmission = (row) => ({
    _id: row._id,
    name: row.name,
...
const createPaperSubmission = async (req, res) => {
    ...
    const paperFileUrl = await (0, upload_service_1.uploadPaperToS3)(req.file);
...
const updatePaperSubmission = async (req, res) => {
    ...
    if (req.file) {
        paperFileUrl = await (0, upload_service_1.uploadPaperToS3)(req.file);
    }
